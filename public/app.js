...
document.querySelector('[data-task="quote"]')?.addEventListener("click",e=>{e.preventDefault();const url=`https://x.com/intent/tweet?text=${encodeURIComponent(quoteText)}&url=${encodeURIComponent(pinnedPost)}`;window.open(url,"_blank","noopener");showCheck("quote");});
tasks.filter(x=>x!=="quote").forEach(name=>document.querySelector(`[data-task="${name}"]`)?.addEventListener("click",()=>showCheck(name)));
...
try{const r=await fetch("/api/submit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({wallet:w})});const d=await r.json();if(!r.ok)throw new Error(d.error||"Submission failed.");message.textContent="WHITELIST SUBMITTED ✓";wallet.value="";}
