...
function clearCheck(name){
  ...
  document.querySelector(`.${name}-check`)?.classList.remove("visible");
}
...
async function loadCount(){
  try{
    ...
    const count=Math.min(Number(d.count)||0,1000);
    liveCount.textContent=`${count.toLocaleString("en-US")} / 1,000`;
  }catch{
    liveCount.textContent="0 / 1,000";
  }
}
loadCount();
